...
newBookForm.addEventListener("submit", (e) => {
  ...
  removeBtn.addEventListener("click", (e) => {
    if (
      removeBtn.getAttribute("data-UUID") === newCard.getAttribute("data-UUID")
    ) {
      newCard.remove();
      const newLibrary = myLibrary.filter(
        (book) => book.UUID !== removeBtn.getAttribute("data-UUID")
      );
  ...
  button.addEventListener("click", (e) => {
    ...
    card.forEach((card) => {
      if (card.getAttribute("data-UUID") === buttonUUID) {
        card.remove();
        const newLibrary = myLibrary.filter(
          (book) => book.UUID !== button.getAttribute("data-UUID")
        );
        myLibrary = newLibrary;
      }
    });
